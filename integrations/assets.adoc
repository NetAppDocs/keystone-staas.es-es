...
NOTE: La página *Activos* muestra pestañas basadas en las plataformas asociadas con sus suscripciones de Keystone . Para ONTAP, están disponibles las pestañas *Nodos en clústeres* y *Volúmenes en clústeres*. Para StorageGRID, la tabla *Nodos en cuadrículas* se muestra directamente sin una selección de pestañas. Si las suscripciones incluyen ambas plataformas, las tres pestañas estarán disponibles.
...
+
Ves la información detallada sobre todos los volúmenes en clústeres a través de las suscripciones de Keystone, incluyendo el nombre del volumen, el número de suscripción, el número de serie del nodo, el cumplimiento con las políticas de QoS, el nombre del clúster, el nombre de host y los niveles de servicio de rendimiento. Puedes monitorear la capacidad aprovisionada, la utilización lógica y física, los datos fríos y los datos calientes. También obtienes una visión general de:
...
NOTE: Las columnas *Cold tier %* y *Hot tier %* muestran el porcentaje de datos en cada nivel, calculado como (Cold data / Total footprint) × 100 y (Hot data / Total footprint) × 100 respectivamente.
