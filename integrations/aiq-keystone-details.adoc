...
La siguiente imagen muestra los gráficos para el nivel de servicio _Extreme_ (nivel de servicio base) y un número de suscripción principal. El gráfico de la misma serie temporal también indica el consumo del sitio de réplica en un tono más claro del mismo código de color utilizado para el sitio principal. La información sobre herramientas al pasar el ratón muestra el desglose de consumo (en TiB) para los sitios principales y de reflejo, 1,02 TiB y 1,05 TiB respectivamente.
