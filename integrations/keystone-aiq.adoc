...
NOTE: El consumo de capacidad en las suscripciones de Keystone se muestra en TiBs en los paneles e informes y se redondea a dos decimales. Si el uso es inferior a 0,01 TiB, el valor se muestra como 0 o Sin uso. Los datos de estas pantallas se muestran en hora UTC (zona horaria del servidor). Cuando introduces una fecha para la consulta, se considera automáticamente que está en hora UTC. Para saber más sobre las métricas de uso, consulta link:../concepts/metrics.html#metrics-measurement["Medición de métricas"]. Para información sobre las diferentes capacidades usadas en Keystone, consulta link:../concepts/supported-storage-capacity.html["Tipos de capacidad de almacenamiento en Keystone"].
