...
[NOTE]
====
* Esta vista está limitada a una suscripción de Keystone a la vez. También puede ver los activos de otras suscripciones seleccionándolos en la lista desplegable *ID de seguimiento*. El panel de Keystone en NetApp Console le permite ver activos en múltiples suscripciones de Keystone . Para obtener más información, consultelink:../integrations/assets.html["Visualiza los activos en todas tus suscripciones de Keystone"] .
* Las opciones del menú desplegable *Ver* dependen de sus plataformas de suscripción de Keystone . Los *volúmenes en clústeres* y los *nodos en clústeres* están disponibles para ONTAP. Para StorageGRID, el menú desplegable *Ver* no está disponible, la tabla *Nodos en cuadrículas* se muestra automáticamente. Si su suscripción incluye ambas plataformas, las tres opciones están disponibles.
...
+
Ves la información detallada sobre los volúmenes en los clústeres, incluyendo el nombre del volumen, el número de serie del nodo, el nombre del clúster, el nombre de host y los niveles de servicio de rendimiento. También puedes monitorear la capacidad aprovisionada, la utilización lógica y física, los datos fríos y los datos calientes.
