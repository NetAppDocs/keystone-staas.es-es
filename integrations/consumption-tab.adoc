...
NOTE: Puedes usar el conmutador *Previsión* para ver el uso de capacidad proyectado en la vista *Consumo histórico* y la ráfaga acumulada proyectada en la vista *Ráfaga acumulada*. Para obtener más información, consulta link:../integrations/consumption-tab.html#view-forecast-data["Ver datos de previsión"].
...
+
image:consumption-trend-burst.png["captura de pantalla que muestra el consumo histórico"]

+
El gráfico lineal muestra datos históricos de consumo y permite a los usuarios analizar tendencias en un intervalo de fechas seleccionado. El gráfico muestra las siguientes métricas:

+
** *Capacidad utilizada*: Almacenamiento total en uso, incluida la capacidad de datos y de instantáneas.
** *Datos*: Capacidad de almacenamiento utilizada por los datos del usuario.
** *Instantánea*: Capacidad de almacenamiento utilizada por los datos de la instantánea.
** *Comprometida*: la capacidad comprometida contratada.
** *Capacidad de ráfagas*: el límite máximo de ráfagas permitido para tu suscripción.
** *Por encima de la capacidad de ráfaga*: Consumo de almacenamiento por encima del límite de ráfaga. Esta métrica aparece solo cuando se supera el límite de ráfaga.
+
Según el intervalo de fechas de tu consulta, los gráficos de uso muestran hasta 30 puntos de recopilación de datos. Puedes pasar el cursor del ratón por encima del gráfico para ver el desglose de uso en cada punto de recopilación de datos.

+

NOTE: El intervalo de fechas que selecciones determina el nivel de detalle mostrado en la parte inferior del gráfico. Para periodos de 30 días o menos, se muestran tanto la fecha como la hora para cada punto de datos. Para periodos superiores a 30 días, solo se muestra la fecha.
...
. Selecciona el intervalo de tiempo desde los iconos de calendario en los campos *From Date* y *To Date*. Por defecto, el intervalo de fechas es de los últimos 30 días desde hoy. Puedes cambiar este intervalo, pero no puedes seleccionar una fecha futura.
+
...
NOTE: Si seleccionas un intervalo de fechas de menos de 30 días, el gráfico muestra un punto de recopilación de datos por día solo para los días seleccionados. Por ejemplo, un intervalo de 4 días muestra 4 puntos de recopilación de datos, no 30. Solo se muestra la fecha en la parte inferior del gráfico para cualquier intervalo seleccionado. Las marcas de tiempo son visibles solo cuando haces zoom en el gráfico usando la función de acercar que se describe abajo.
...
En la Consola, cuando el conmutador *Previsión* está desactivado, puedes ver el uso de ráfagas facturado junto con el uso de ráfagas no facturado (acumulado pero aún no facturado) y el uso de ráfagas provisional (acumulado en el periodo de facturación actual y aún no facturado). Cuando el conmutador *Previsión* está activado, el gráfico también muestra el uso de ráfagas acumulado proyectado para los próximos 120 días a partir de la fecha actual. Para saber más sobre el conmutador *Previsión*, consulta link:../integrations/consumption-tab.html#view-forecast-data["Ver datos de previsión"].

image:accr-burst-provision-invoice.png["captura de pantalla que muestra el uso acumulado de ráfagas en la Console"]
...
== Ver datos de previsión

En la Consola, puedes usar el conmutador *Previsión* en la pestaña *Tendencia de consumo* para ver el uso de capacidad proyectado en la vista *Consumo histórico* y la ráfaga acumulada proyectada en la vista *Ráfaga acumulada*. El conmutador está activado por defecto en ambas vistas.

En la vista *Consumo histórico*, el gráfico muestra los últimos 30 días de datos históricos y una proyección de 120 días a partir de la fecha actual en el mismo gráfico. Se trazan hasta 30 puntos de datos en total (20 para datos históricos y 10 para datos de previsión), con una línea vertical que separa los dos segmentos.

image:forecast-1.png["captura de pantalla con datos de previsión"]

En la vista *Ráfaga acumulada*, el gráfico muestra los últimos 6 meses de historial de ráfagas acumuladas y una proyección de 120 días a partir de la fecha actual, con una línea vertical que separa los datos históricos de los datos previstos. La barra del periodo de facturación actual muestra tanto el uso provisional de ráfagas como los datos de previsión para los días restantes. Puedes hacer clic en cualquier barra para ver el desglose día a día en la tabla *Ráfagas acumuladas por días*. Para saber más sobre la tabla *Ráfagas acumuladas por días*, consulta link:../integrations/consumption-tab.html#view-daily-accrued-burst-data-usage["Ver el uso diario de datos de ráfaga acumulados"].

image:accrued-burst-forecast.png["captura de pantalla de la previsión de ráfagas acumuladas"]

Si seleccionas un intervalo de fechas que no termina en la fecha actual, la previsión se desactiva automáticamente. Para volver a activarla, un cuadro de confirmación te informa que los datos de previsión solo están disponibles cuando la línea de tiempo seleccionada termina en la fecha actual. Si confirmas el cuadro de diálogo, la vista se restablece al intervalo predeterminado para la vista seleccionada: los últimos 30 días de datos históricos más los próximos 120 días de previsión en la vista *Consumo histórico*, o los últimos 6 meses de historial de ráfagas acumuladas más los próximos 120 días de previsión en la vista *Ráfagas acumuladas*.

Si se espera que el consumo previsto supere la capacidad comprometida, aparece un indicador de alerta naranja en el gráfico, recomendando una ampliación de la capacidad. Si se espera que supere la capacidad de ráfaga, también aparece un indicador de alerta rojo, recomendando una ampliación de la capacidad.
...
NOTE: Los datos mostrados en el gráfico dependen de si la opción *Forecast* está activada o desactivada.

image:bxp-accrued-burst-days-1.png["Captura de pantalla que muestra la tabla de ráfagas acumuladas por días"]

También puedes descargar los datos disponibles en formato Excel, incluido el estado de las ráfagas (facturadas, no facturadas, provisionales o previstas, según corresponda) y el periodo de facturación.
